...
prerequisites: 无
relateddocuments: [ai辅助代码目录结构.md, Function Testing Work ProcessCapgeminiV1.md, JTPPostman.md]
...
[术语认知] 功能测试基本术语定义：功能测试、冒烟测试、Feature testI-Test)、SIT、UAT、Rehearsal test、Go-live test、回归测试
...
B1_intro = """B1（工程师）能力要求汇总
skilllevel: B1
position: 工程师
role: 独立执行者
focusareas: [测试执行, 环境搭建, 基础脚本]
keyskills:
...
relateddocuments: [ai辅助代码目录结构.md, Function Testing Work ProcessCapgeminiV1.md, JTPjmeter.md, JTPPostman.md, JTPSQL基础入门文档.md, pythonUI自动化.md, python接口自动化.md]
keywords: [common.py, obtainfilejson.py, 前置条件, 启动检查, JMeter安装, 线程组, HTTP请求, Postman变量, SQL连接, NULL值, Selenium环境, requests.get]"""

B1_21 = """技能类别：测试执行与基础自动化

检索关键词: common.py函数, obtainfilejson.py, 测试前置条件, 测试启动检查, JMeter安装, JMeter线程组, Postman变量, SQL连接类型, NULL值处理

特定能力详情
AI辅助代码目录结构
[工具使用] 能使用 common.py 中的基础函数：loadswaggerfile, extractdata, replacekeyvalues, replaceslashwithdot, findvaluebykey, checkandcreatedirectory, removekey, extractdescriptionkeys, getstr
[工具使用] 能运行 execute 层的 obtainfilejson.py 文件，通过 createsubdirectoriesfromdictkeys、getanydatas、processdict、getjsonpointer、generatefeature 等方法解析 Swagger 文件并生成目录结构
Function Testing Work Process
...
B1_22 = """检索关键词: UI自动化步骤, Selenium环境搭建, Selenium基础操作, 接口测试流程, requests.get
...
[工具操作] 能使用 Selenium 初始化浏览器对象、访问页面、设置浏览器大小、前进后退、获取页面基础属性（title, currenturl, name, pagesource）
...
B2_intro = """B2（中级工程师）能力要求汇总
skilllevel: B2
position: 中级工程师
role: 熟练执行者
focusareas: [测试执行熟练, 工具进阶, 脚本编写]
keyskills:
...
[工具操作] 能使用 Newman 命令行执行 Postman Collection：newman run v2ex.postmancollection.json -d test.json -r html,cli,json,junit
...
B2_32 = """检索关键词: 接口参数关联, token传递, 加密接口, MD5签名, YAML数据结构, YAML语法, 数据驱动测试
...
B2_33 = """检索关键词: Postman Tests断言, Postman Cookies, Postman鉴权, Pre-request Script, Collection参数化, 脚本导出
...
C1_intro = """C1（高级工程师）能力要求汇总
skilllevel: C1
position: 高级工程师
role: 框架使用者
focusareas: [pytest框架, 接口自动化框架, UI自动化进阶, 测试用例设计]
keyskills:
...
relateddocuments: [pytest框架.md, python接口自动化.md, pythonUI自动化.md, Function Testing Work ProcessCapgeminiV1.md]
keywords: [pytest, pytest插件, pytest.ini, @pytest.mark, fixture, 参数化, assert, requests.post, YAML, findelement, ActionChains, Keys, 测试用例设计原则]"""

C1_41 = """技能类别：自动化框架与中级测试能力
...
[规则认知] 了解 pytest 默认测试用例发现规则：模块名以 test_ 开头或 test 结尾；测试类以 Test 开头且不能有 init 方法；测试方法以 test 开头
...
[工具操作] 能使用 setup/teardown, setupclass/teardownclass 实现用例前后置
...
C1_42 = """检索关键词: 接口自动化框架结构, requests.post, 响应内容获取, common层, data层, YAML
...
[工具操作] 能获取响应内容：statuscode, url, encoding, headers, cookies, text, content, json()

分块说明: 此块从使用单个请求库升级到理解和搭建完整的接口自动化测试框架，是工程化测试的开始。"""

C1_43 = """检索关键词: Selenium元素定位, ActionChains鼠标操作, Keys键盘操作, 等待机制, 测试用例设计原则, 4-eyes-principle

特定能力详情
Python (UI 自动化进阶)
[工具操作] 能使用 Selenium 的 findelement 方法通过不同属性定位元素：CLASS, XPATH, LINKTEXT, PARTIALLINKTEXT, TAG, CSS, ID
[工具操作] 能使用 ActionChains 模拟鼠标操作：右击(contextclick)、双击(doubleclick)、拖拽(draganddrop)、悬停(movetoelement)
[工具操作] 能使用 Keys 类模拟键盘操作：删除键、空格键、制表键、回退键、回车、全选、复制、剪切、粘贴、F1 等
[知识认知] 了解 Selenium 三种等待方式：强制等待（time.sleep）、隐式等待（implicitlywait）、显式等待
...
C2_intro = """C2（资深工程师）能力要求汇总
skilllevel: C2
position: 资深工程师
role: 策略制定者
focusareas: [框架优化, 测试策略, 测试管理, SQL进阶, 流程管理, 缺陷管理, 测试报告, 自动化价值分析, 质量保障体系, 上线后支持, 高级测试技术, AI辅助测试]
keyskills:
了解AI辅助框架原理并能操作
能制定测试计划、测试策略和风险评估
...
relateddocuments: [ai辅助代码目录结构.md, ai辅助操作手册.md, Function Testing Work ProcessCapgeminiV1.md, JTPSQL基础入门文档.md, pytest框架.md, pythonUI自动化.md, python接口自动化.md]
...
C2_51 = """技能类别：框架优化与测试策略设计

检索关键词: $ref嵌套解析, AI辅助框架步骤, 测试计划制定, 测试策略, 风险评估, 冒烟测试失败, 提测延误, 缺陷阻塞

特定能力详情
AI辅助代码目录结构, AI辅助操作手册 (框架扩展与维护)
[原理认知] 了解 obtainfilejson.py 中的 processdict 方法支持解析 "$ref" 最多三层嵌套，后续可能优化
[工具认知] 了解 extractfilename.py 和 outputpromt.py 在新版中未使用
...
C2_52 = """检索关键词: 测试用例评审, 冒烟测试执行, I-Test执行, SIT执行, UAT执行, Rehearsal test, Go-live test, 回归测试场景

特定能力详情
Function Testing Work ProcessCapgemini (测试执行与管理)
...
C2_53 = """检索关键词: 主键必要性, 数据唯一性, 索引, GROUP BY规范, 分组字段, 聚合函数
...
C2_55 = """检索关键词: Test Report, 复盘总结, UI自动化优点, UI自动化挑战, 接口自动化分类, 接口自动化性价比
...
C2_57 = """检索关键词: Allure报告, Jenkins集成, Selenium特点, 分布式测试, AI辅助框架, execut层, swagger层
...
db = [
    {"label": "A5", "content": A5_intro},
    {"label": "A5", "content": A5_11},
    {"label": "A5", "content": A5_12},
    {"label": "B1", "content": B1_intro},
    {"label": "B1", "content": B1_21},
    {"label": "B1", "content": B1_22},
    {"label": "B2", "content": B2_intro},
    {"label": "B2", "content": B2_31},
    {"label": "B2", "content": B2_32},
    {"label": "B2", "content": B2_33},
    {"label": "C1" , "content": C1_intro},
    {"label": "C1", "content": C1_41},
    {"label": "C1", "content": C1_42},
    {"label": "C1", "content": C1_43},
    {"label": "C2", "content": C2_intro},
    {"label": "C2", "content": C2_51},
    {"label": "C2", "content": C2_52},
    {"label": "C2", "content": C2_53},
    {"label": "C2", "content": C2_54},
    {"label": "C2", "content": C2_55},
    {"label": "C2", "content": C2_56},
    {"label": "C2", "content": C2_57}
]

valid_levels = ["A5", "B1", "B2", "C1", "C2"]

def get_level(user_level):
    while True:
        level = user_level.strip().upper()
...
def main(user_level):
    selected_level = get_level(user_level)

    filtered_content = filter(lambda d: d["label"] == selected_level, db)
